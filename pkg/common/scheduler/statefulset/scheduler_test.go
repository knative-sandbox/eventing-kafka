...
func TestStatefulsetScheduler(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.schedulerPolicyType == scheduler.EVENSPREAD || tc.schedulerPolicyType == scheduler.EVENSPREAD_BYNODE {
			...
			sa := state.NewStateBuilder(ctx, vpodClient.List, 10, tc.schedulerPolicyType, lsn.GetNodeLister())
